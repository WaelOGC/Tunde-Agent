...
from tunde_agent.services.telegram_pending_email import clear_pending_report_email
from tunde_agent.services.telegram_pending_landing_design import clear_pending_landing_design
from tunde_agent.services.telegram_pending_photo_edit import clear_pending_photo_edit
from tunde_agent.services.telegram_pending_video_generation import (
    clear_pending_video_preset,
    set_pending_video_preset,
)
from tunde_agent.services.telegram_report_context import clear_report_chat_context
...
def about_message_html() -> str:
    return (
        ...
        "<b>Veo video</b> presets (10 / 20 / 30 tiers under Pro Video). Image-to-motion “Animate image” "
            ...
            if key in ("10", "20", "30"):
                if not (settings.gemini_api_key or "").strip():
                    _edit_menu_message(
                        ...
                        chat_id_int,
                        message_id_int,
                        "<b>Video generation</b>\n\nSet <code>GEMINI_API_KEY</code> for this bot to use Veo."
                        + BRANDING_FOOTER_HTML,
                        back_markup,
                    )
                    return
