...
**Code:** `telegram_agent_core/services/telegram_chat_handler.py` (routing + pending), `telegram_agent_core/services/telegram_pending_photo_edit.py`, `telegram_agent_core/services/telegram_service.py` (`fetch_telegram_file_bytes`), `tunde_agent/services/gemini_image_generation.py` (`generate_image_from_reference_bytes`).

---

## 2. Pro video generation (**active**)

Telegram **Pro Video Generation** uses a server-side **video generation engine** (async job + polling + download). Env: `GEMINI_API_KEY`, `GEMINI_VIDEO_MODEL` (default `veo-3.1-generate-preview`), optional `TUNDE_VEO_*` — see [`.env.example`](../.env.example). User-facing Telegram copy must not mention provider names.
...
The underlying API delivers **4 / 6 / 8** second **segments** per generation; **extension** adds about **7 seconds** per step at **720p**. Product tiers map as follows:
...
Tiers **20** and **30** always use **720p** for the base clip so **video-to-video extension** stays within the vendor’s rules and limits.
...
Compliance: follow [../02_web_app_backend/security_and_legal_compliance.md](../02_web_app_backend/security_and_legal_compliance.md) and [human_approval_gate.md](./human_approval_gate.md) for prompts involving real people, brands, or sensitive contexts.

**Code:** `tunde_agent/services/gemini_veo_video.py`, `telegram_agent_core/services/telegram_pending_video_generation.py`, `telegram_agent_core/services/telegram_ux_menus.py` (`v:` callbacks), `telegram_agent_core/services/telegram_chat_handler.py`, `telegram_agent_core/services/telegram_service.py` (`send_video_to_chat`).

---

## 3. Voiceover synthesis (roadmap)

**Planned:** optional narration track aligned to video duration presets (10 / 20 / 30 s), with language and tone chosen after script approval. Not shipped in the current tree — see [../05_project_roadmap/roadmap.md](../05_project_roadmap/roadmap.md).

---

## 4. PDF export

**Deep research delivery:** the report message includes **📄 PDF** and **📥 Export to PDF** (same underlying export). Professional PDF layout is described under reporter / post-task handlers in code; roadmap item [../05_project_roadmap/roadmap.md](../05_project_roadmap/roadmap.md) tracks further productization.
