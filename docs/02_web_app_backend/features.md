...
Capability catalog for Tunde Agent’s initial product surface, described in acceptance-oriented terms. System shape and boundaries are in [architecture.md](./architecture.md). Deployment and environments are in [infrastructure.md](./infrastructure.md). Deferred work and phasing appear in [../05_project_roadmap/roadmap.md](../05_project_roadmap/roadmap.md). Safety and self-improvement constraints are in [../05_project_roadmap/self_improvement_rules.md](../05_project_roadmap/self_improvement_rules.md).
...
- **Human approval gate** — **Sending** requires an explicit user confirmation in MVP (no unattended mass send). Scheduled or batched send, if added later, remains policy-governed per [../05_project_roadmap/self_improvement_rules.md](../05_project_roadmap/self_improvement_rules.md).
...
- **Safety** — Aligns with the risk themes in [../05_project_roadmap/roadmap.md](../05_project_roadmap/roadmap.md) and the enforcement model in [../05_project_roadmap/self_improvement_rules.md](../05_project_roadmap/self_improvement_rules.md).
...
The following are **out of scope** for the initial web MVP and are tracked in [../05_project_roadmap/roadmap.md](../05_project_roadmap/roadmap.md):
...
**Web dashboard (workspace):** A Vite + React app lives in **`tunde_webapp_frontend/`** with a FastAPI companion in **`tunde_webapp_backend/`** (tasks, tools, file upload, image generation, landing pages). Operator tools, env vars, and flows are documented in [../03_web_app_frontend/workspace_tools_and_landing.md](../03_web_app_frontend/workspace_tools_and_landing.md). This is distinct from the main **`tunde_agent`** API on port **8000** by default (the webapp backend often runs on **8001** via `TUNDE_WEBAPP_PORT`).
