...
from tunde_agent.services.telegram_pending_landing_design import (
    clear_pending_landing_design,
    get_pending_landing_design,
)
from tunde_agent.services.telegram_pending_photo_edit import (
    clear_pending_photo_edit,
    get_pending_photo_edit_file_id,
    set_pending_photo_edit_file_id,
)
from tunde_agent.services.telegram_pending_video_generation import (
...
from tunde_agent.services.telegram_ux_pending import (
...
def _help_message_html() -> str:
    return (
        ...
        "Send <b>/start</b> anytime to open the main pillar menu. <b>Pro Video</b>: pick 10 / 20 / 30, "
        "then describe your scene in the next message. Cancel a waiting scene with <code>/cancel_video</code>. "
        "After a report arrives, use the action row on that message for exports and follow-ups.\n\n"
...
def process_incoming_message(msg: dict, settings: Settings | None = None) -> None:
    ...
    if get_pending_video_preset(cid_int) and has_visual:
        tg.send_message_to_chat(
            cid_int,
            "I’m waiting for your <b>video scene description</b> as text — or say <code>/cancel_video</code>. 💛",
            parse_mode="HTML",
        )
        return
    ...
    if low.startswith("/help") or low.startswith("/help@"):
        tg.send_message_to_chat(cid_int, _help_message_html(), parse_mode="HTML")
        return
    ...
    if low == "/cancel_landing" or low.startswith("/cancel_landing@"):
        if get_pending_landing_design(cid_int):
            clear_pending_landing_design(cid_int)
            tg.send_message_to_chat(cid_int, "Cancelled — I won’t wait for a landing design on that report. 💛")
        else:
            tg.send_message_to_chat(cid_int, "No custom landing design was waiting. 💛")
        return

    if low == "/cancel_photo_edit" or low.startswith("/cancel_photo_edit@"):
        if get_pending_photo_edit_file_id(cid_int):
            clear_pending_photo_edit(cid_int)
            tg.send_message_to_chat(cid_int, "Cancelled — photo edit cleared. 💛")
        else:
            tg.send_message_to_chat(cid_int, "No photo edit was waiting for instructions. 💛")
        return

    if _telegram_command_stem(text) == "/cancel_video":
        if get_pending_video_preset(cid_int):
            clear_pending_video_preset(cid_int)
            tg.send_message_to_chat(cid_int, "Cancelled — video preset cleared. 💛")
        else:
            tg.send_message_to_chat(cid_int, "No video scene was waiting. 💛")
        return
    ...
    if m_cmd:
        ...
        if not topic:
            tg.send_message_to_chat(
                cid_int,
                "Tell me what to research — pick a pillar from <b>/start</b> or add your topic after "
                "<code>/research</code>. 🌸",
                parse_mode="HTML",
            )
            return
