...
Related: [../01_telegram_bot/human_approval_gate.md](../01_telegram_bot/human_approval_gate.md) for pausing sensitive actions; [security_and_legal_compliance.md](./security_and_legal_compliance.md) for legal and scraping boundaries.
...
2. **Notify the user immediately** — Same channels as sensitive approvals where practical: **Web UI** and optionally **Telegram** (see [../01_telegram_bot/human_approval_gate.md](../01_telegram_bot/human_approval_gate.md)), with a **clear link** or **session context** so the user can complete the challenge in a controlled way.
