// tunde_webapp_frontend/src/components/WebPageDesignerCanvas.jsx
import { useState } from "react";

const TABS = ["Preview", "Code", "Export"];

export default function WebPageDesignerCanvas({ page, onClose }) {
  const [activeTab, setActiveTab] = useState("Preview");
  const [copied, setCopied] = useState(false);

  if (!page) return null;

  const { business_name, page_title, html_content, industry,
          page_style, color_scheme, sections, created_at } = page;
  ...
  const downloadHtml = () => {
    ...
    a.href     = url;
    a.download = `${business_name.replace(/\s+/g, "_")}_page.html`;
    a.click();
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-500 text-xs">
              {industry} · {page_style} · {color_scheme}
            </p>
          </div>
        </div>
        ...
        {activeTab === "Code" && (
          <div className="h-full flex flex-col">
            <div className="flex items-center justify-between px-4 py-2 border-b border-white/5 shrink-0"
                 style={{ background: "#0a0f1a" }}>
              <span className="text-xs text-gray-500 font-mono">HTML — self-contained</span>
              <button
            ...
            <div className="flex-1 overflow-auto p-4">
              <pre
                className="text-xs text-gray-300 font-mono leading-relaxed whitespace-pre-wrap"
                style={{ tabSize: 2 }}
              >
                {html_content}
              </pre>
            ...
            <div className="grid grid-cols-1 gap-4 max-w-lg">
              ...
              <div className="rounded-xl border border-white/10 p-5"
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-white font-semibold text-sm">HTML File</p>
                    <p className="text-gray-500 text-xs mt-0.5">
                      Complete self-contained page — ready to deploy
                    </p>
                  </div>
                  <button
                    onClick={downloadHtml}
              ...
              <div className="rounded-xl border border-white/10 p-5"
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-white font-semibold text-sm">Copy HTML</p>
                    <p className="text-gray-500 text-xs mt-0.5">
                      Paste directly into your CMS or editor
                    </p>
                  </div>
                  <button
                    onClick={() => copyToClipboard(html_content)}
              ...
              <div className="rounded-xl border border-white/10 p-5"
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-white font-semibold text-sm">Preview in Browser</p>
                    <p className="text-gray-500 text-xs mt-0.5">
              ...
              {/* Meta */}
              <div className="rounded-xl border border-white/5 p-5 mt-2"
                   style={{ background: "#0a0f1a" }}>
